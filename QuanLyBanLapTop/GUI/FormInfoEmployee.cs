...
namespace GUI
{
    public partial class FormInfoEmployee : Form
    {
        ...
        FormEmployee femployee;
        public FormInfoEmployee(string idEmployee)
        {
            InitializeComponent();
            this.idEmployee = idEmployee;
            initializationDataBill();
            initializationDetails();
            initializationInfoEmployee();
        }
        public void initializationDataBill()
        {
            ...
            autoSizetb(tb_bill);
            setDataBill(employeeManage.searchBillInEmployee(this.idEmployee));
        ...
        public void initializationInfoEmployee()
        {
            NhanVien employee = employeeManage.searchIdEmployee(this.idEmployee);
            txt_id.Text = employee.Id;
            txt_name.Text = employee.Name;
            txt_phone.Text = employee.Phone;
            txt_service.Text = employee.ServiceEmployee;
            txt_salary.Text = employee.SalaryEmployee.ToString("0.0");
        }
        ...
        private void inp_search_TextChanged_1(object sender, EventArgs e)
        {
            string text = this.inp_search.Text;
            if (text == "")
            {
                List<HoaDon> list = billManage.searchBill(text, employeeManage.searchBillInEmployee(this.idEmployee));
                setDataBill(list);
            }
        }
